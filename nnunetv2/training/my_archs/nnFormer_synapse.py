"""
Synapse 版本与 nnFormer_acdc.py 的主干结构基本一致。
如果你是为了学习主干线代码、跳跃连接以及 3D 局部/全局注意力，
优先阅读已经补充详细中文注释的 nnFormer_acdc.py，会更容易顺着 forward 看懂。
"""
...
class SwinTransformerBlock_kv(nn.Module):
    """
    跳跃连接中，作为选择性接受的Transfomrer模块，q来源于解码器上一层的输出，k和v来源于编码器对应层的输出
    """
...
class WindowAttention_kv(nn.Module):
    """
    窗口级别跳跃连接中，作为选择性接受的Transfomrer模块，q来源于解码器上一层的输出，k和v来源于编码器对应层的输出
    """
